...
# Usage: curl -fsSL https://raw.githubusercontent.com/bhpark1013/claudenews/main/install.sh | bash

set -e

HUD_DIR="$HOME/.claude/hud"
HUD_FILE="$HUD_DIR/claudenews-hud.mjs"
SETTINGS="$HOME/.claude/settings.json"
RAW_BASE="https://raw.githubusercontent.com/bhpark1013/claudenews/main"

echo ""
echo "  claudenews — status line wrapper setup"
echo "  --------------------------------------"
echo ""
...
if [ ! -d "$PLUGIN_DIR_PATTERN" ]; then
  ...
  echo "  Install the plugin first: /plugin marketplace add bhpark1013/claudenews && /plugin install claudenews"
  echo ""
fi
...
mkdir -p "$HUD_DIR"
curl -fsSL "$RAW_BASE/plugin/hud/claudenews-hud.mjs" -o "$HUD_FILE"
chmod +x "$HUD_FILE"
...
if "claudenews-hud" not in current_cmd:
...
echo "         /plugin marketplace add bhpark1013/claudenews"
echo "         /plugin install claudenews@claudenews"
